...
async def add_task(user_id: str, task_data: dict = Body(...), current_user: str = Depends(get_current_user), session: AsyncSession = Depends(get_session)):
    try:
        ...
        # Auto-generate UUID for task id if not provided
        task_id = task_data.pop("id", str(uuid.uuid4()))
        
        task = Task(id=task_id, user_id=user_id, **task_data)
        session.add(task)
...
async def update_task(user_id: str, task_id: str, task_data: dict = Body(...), current_user: str = Depends(get_current_user), session: AsyncSession = Depends(get_session)):
    if current_user != user_id:
        raise HTTPException(status_code=403, detail="Not authorized")
...
async def mark_complete(user_id: str, task_id: str, data: CompleteUpdate, current_user: str = Depends(get_current_user), session: AsyncSession = Depends(get_session)):
    if current_user != user_id:
        raise HTTPException(status_code=403, detail="Not authorized")
    stmt = select(Task).where(Task.id == task_id, Task.user_id == user_id)
    result = await session.execute(stmt)
...
async def delete_task(user_id: str, task_id: str, current_user: str = Depends(get_current_user), session: AsyncSession = Depends(get_session)):
    if current_user != user_id:
        raise HTTPException(status_code=403, detail="Not authorized")
    stmt = select(Task).where(Task.id == task_id, Task.user_id == user_id)
    result = await session.execute(stmt)
